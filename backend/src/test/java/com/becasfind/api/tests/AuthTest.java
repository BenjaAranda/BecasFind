...
class AuthTest extends BaseTest {
    ...
    @Test
    @DisplayName("CP-01: Login exitoso con credenciales validas")
    void loginSuccess() {
        var res = post("/api/auth/login", null,
                Map.of("email", "admin@becasfind.cl", "password", "admin123"), Map.class);
    ...
    @Test
    @DisplayName("CP-02: Login fallido - contrasena incorrecta")
    ...
    @Test
    @DisplayName("CP-03: Login fallido - email no registrado")
    ...
    @Test
    @DisplayName("CP-04: Login fallido - campos vacios")
    ...
    @Test
    @DisplayName("CP-05: Login fallido - email invalido")
    ...
    @Test
    @DisplayName("CP-06: Registro exitoso con datos validos")
    void registerSuccess() {
        var res = post("/api/auth/register", null,
                Map.of("nombreCompleto", "Test User", "email", "nuevo@test.cl", "password", "password123"), Map.class);
    ...
    @Test
    @DisplayName("CP-07: Registro fallido - email duplicado")
    void registerDuplicateEmail() {
        var res = post("/api/auth/register", null,
                Map.of("nombreCompleto", "X", "email", "admin@becasfind.cl", "password", "password123"), Map.class);
        assertTrue(res.getStatusCodeValue() >= 400);
    }

    @Test
    @DisplayName("CP-08: Registro fallido - contrasena corta")
    void registerShortPassword() {
        var res = post("/api/auth/register", null, Map.of("nombreCompleto", "X", "email", "x@x.cl", "password", "123"),
                Map.class);
        assertEquals(400, res.getStatusCodeValue());
    }

    @Test
    @DisplayName("CP-09: Registro fallido - email invalido")
    void registerInvalidEmail() {
        var res = post("/api/auth/register", null,
                Map.of("nombreCompleto", "X", "email", "bad", "password", "password123"), Map.class);
        assertEquals(400, res.getStatusCodeValue());
    }

    @Test
    @DisplayName("CP-10: Registro fallido - campos vacios")
    void registerEmptyFields() {
        var res = post("/api/auth/register", null, Map.of("nombreCompleto", "", "email", "", "password", ""),
                Map.class);
        assertEquals(400, res.getStatusCodeValue());
    }

    @Test
    @DisplayName("CP-11: Recuperacion - envio exitoso")
    void forgotPassword() {
    ...
    @Test
    @DisplayName("CP-16: Reset password - contrasena corta")
    void resetPasswordShort() {
        var res = post("/api/auth/reset-password", null, Map.of("token", "fake-token", "newPassword", "123"),
                Map.class);
        assertEquals(400, res.getStatusCodeValue());
    }
}
